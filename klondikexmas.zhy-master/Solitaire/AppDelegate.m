...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    if (localArr) {
        for (UILocalNotification *noti in localArr) {
            ...
            if (dict) {
                NSString *inKey = [dict objectForKey:@"KlondikeSolitaire"];
                if ([inKey isEqualToString:@"KlondikeSolitaire"]) {
                    localNoti = noti;
                    break;
                }
            }
    ...
    if (noti) {
        ...
        noti.alertBody = NSLocalizedStringFromTable(@"pushmsg", @"Language", nil);
        noti.applicationIconBadgeNumber = 1;
        //for remove
        NSDictionary *infoDic = [NSDictionary dictionaryWithObject:@"KlondikeSolitaire" forKey:@"KlondikeSolitaire"];
   ...
   // [Flurry startSession:@"VZV4H272MYN2THBPG39G"];
...
- (void)application:(UIApplication *)application didReceiveLocalNotification:(UILocalNotification *)notification
{
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@""
                                                   ...
                                                   delegate:nil
                                          cancelButtonTitle:NSLocalizedStringFromTable(@"ok", @"Language", nil)
