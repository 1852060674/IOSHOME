...
//    adView.frame = CGRectMake(0, 0, width, height);
    ...
    if(bannerResponse != nil) {
        [adView setLocalExtraParameterForKey:@"amazon_ad_response" value:bannerResponse];
    } else if(bannerErrorInfo != nil) {
        [adView setLocalExtraParameterForKey:@"amazon_ad_error" value:bannerErrorInfo];
    }
...
{
    if(applovin_initialized && !interCallback.isInUse) {
...
- (void)didLoadAd:(MAAd *)ad
{
    if([ad.adUnitIdentifier isEqualToString: banner_id]) {
        ...
        NSLog(@"[ADUNION] Applovin load Ad from %@, %@", ad.networkName, ad.networkPlacement);
        if(self.delegate != NULL) {
...
- (void)onInterAPSSuccess:(DTBAdResponse *)adResponse {
    ...
    if(inter_delay_init && applovin_initialized && ![inter_id isEqualToString:@""]) {
        inter_delay_init = false;
        if(inter_id && ![inter_id isEqualToString:@""]) {
            [self createInterstitialAd];
        }
