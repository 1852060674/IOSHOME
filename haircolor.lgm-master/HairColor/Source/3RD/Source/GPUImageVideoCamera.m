...
- (void)removeInputsAndOutputs;
{
    ...
    if (videoInput) {
        ...
        videoInput = nil;
        videoOutput = nil;
        [_captureSession removeOutput:stillImageOutput];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    else
    {
        ...
        runAsynchronouslyOnVideoProcessingQueue(^{
            ...
            if ([self.delegate respondsToSelector:@selector(GPUImageVideoCamera:willOutputSampleBuffer:)])
            {
                [self.delegate GPUImageVideoCamera:self willOutputSampleBuffer:sampleBuffer];
            }
...
-(void)takePhoto
{
    ...
    if ([self.delegate respondsToSelector:@selector(GPUImageVideoCameraWillBeginTakePhoto:)]) {
        [self.delegate GPUImageVideoCameraWillBeginTakePhoto:self];
    }
    ...
    [stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        if ([self.delegate respondsToSelector:@selector(GPUImageVideoCamera:didEndTakePhoto:)]) {
            [self.delegate GPUImageVideoCamera:self didEndTakePhoto:image];
        }
    }];
}
...
-(BOOL)hasFlash
{
    return [self.inputCamera hasFlash];
}

-(BOOL)flashModeAvailable:(AVCaptureFlashMode)mode
{
    return [self hasFlash] && [self.inputCamera isFlashAvailable] && [self.inputCamera isFlashModeSupported:mode];
}

-(void)setFlashMode:(AVCaptureFlashMode)mode
{
    if ([self flashModeAvailable:mode]) {
...
-(BOOL)torchModeAvailable:(AVCaptureTorchMode)mode
{
    return [self hasTorch] && [self.inputCamera isTorchAvailable] && [self.inputCamera isTorchModeSupported:mode];
}

-(void)setTorchMode:(AVCaptureTorchMode)mode
{
    if ([self torchModeAvailable:mode]) {
        if ([self.inputCamera lockForConfiguration:nil]) {
