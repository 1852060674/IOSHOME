...
didFailToReceiveAdWithError:(nonnull NSError *)error {
    if(bannerReady != AD_LOADED) {  //if has loaded then alway can show
        ...
        NSLog(@"Failed to receive banner idx %d with error: %@", idx, [error localizedFailureReason]);
        ...
        NSLog(@"Failed to refresh banner idx %d with error: %@", idx, [error localizedFailureReason]);
    }
...
-(void) checkManulRefresh {
    ...
    NSLog(@"admob banner %d 已展示 %lds", idx, showtime);
