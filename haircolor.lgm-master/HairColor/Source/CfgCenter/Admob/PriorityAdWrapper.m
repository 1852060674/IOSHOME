...
-(int) getFirstBannerAd
{
    ...
    for(int i =0; i < [adlist count]; i++)
    {
        ...
        if(status == AD_LOADED)
        {
            NSLog(@"idx %d -------Show Priority Ad %d", idx, i);
            return i;
        }
    ...
    if(firstinit != -1)
    {
        NSLog(@"idx %d-------Show Priority Banner Ad %d", idx, firstinit);
        return firstinit;
    }
    if(firstSuccessedBefore != -1)
    {
        NSLog(@"idx %d-------Show Priority Banner Ad %d [all failed & choose success before]", idx, firstSuccessedBefore);
        return firstSuccessedBefore;
    }
    
    if(firstsupport != -1) {
        NSLog(@"idx %d-------Show Priority Banner Ad %d [all failed]", idx, firstsupport);
        return firstsupport;
    }
    return -1;
}
...
-(ADWrapper* ) getFirstNativeAd
{
    ...
    for(int i =0; i < [adlist count]; i++)
    {
        ...
        if(status == 1)
        {
            NSLog(@"-------Show Priority Native Ad %d", i);
            currentNativeShow = i;
    ...
    if(firstinit != -1)
    {
        NSLog(@"-------Show Priority Ad Native %d", firstinit);
...
-(ADWrapper* ) getFirstInterstitialAd:(int) place
{
    ...
    for(int i =0; i < [adlist count]; i++)
    {
        ...
        {
            NSLog(@"-------Show Priority Inter Ad %d", i);
            return [adlist objectAtIndex:i];
        }
    }
    return nil;
...
//        }
//    }
//    NSLog(@"Banner count %ld, %d个前台:%@", [adlist count], forecount, str);
}
