...
#ifdef LOG_USER_ACTION
            //[Flurry logEvent:@"RT Return" withParameters:@{@"Accept":@(1)}];
#endif
...
-(void) showRT: (UIViewController *)viewCtrl
{
    ...
    UIAlertView *rateDlg = [[UIAlertView alloc]
                            initWithTitle:@""
                            message:NSLocalizedStringFromTable(@"RATE_MSG", @"RTService", nil)
                            delegate:self cancelButtonTitle:NSLocalizedStringFromTable(@"RATE_CANCEL", @"RTService", nil)
                            otherButtonTitles:NSLocalizedStringFromTable(@"RATE_YES", @"RTService", nil), NSLocalizedStringFromTable(@"RATE_NO", @"RTService", nil), nil];
    rateDlg.tag = REVIEW_REQUEST_ALERTVIEW_TAG;
    [rateDlg show];
#ifdef LOG_USER_ACTION
    //[Flurry logEvent:@"RT Return" withParameters:@{@"Show":@(0)}];
#endif
}
...
- (BOOL) doFeedback: (UIViewController *)viewCtrl
{
    ...
    if (send == NO) {
        [[viewCtrl view] makeToast:NSLocalizedStringFromTable(@"EMAIL_NOT_AVAILABLE", @"RTService", @"") duration:1.5 position:@"center"];
    }
    return send;
}
...
-(NSString *)getSettingFeedbackMessage
{
    ...
    NSString *message = [NSString stringWithFormat:@"%@<br />%@:%@<br />%@:%@<br />%@:%@<br />%@:%.2f",
                         NSLocalizedStringFromTable(@"SETTING_FEEDBACK_CONTENT", @"RTService", @""),
                         NSLocalizedStringFromTable(@"SETTING_APP_NAME_WORD", @"RTService", @"app name for locale"),
                         appname,
                         NSLocalizedStringFromTable(@"SETTING_APPVERSION_WORD", @"RTService", @"app version for locale"),
                         appversion,
                         NSLocalizedStringFromTable(@"SETTING_DEVICE_WORD", @"RTService", @"device for locale"),
                         [self getDeviceModel],
                         NSLocalizedStringFromTable(@"SETTING_IOS_VERSION_WORD", @"RTService", @"ios version locale"),
                         [self systemVersion]
                         ];
    return message;
}
...
-(NSString *)getSettingFeedbackTitle
{
    ...
    if(appname == nil)
        appname = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleDisplayName"];
    NSString *title = [NSString stringWithFormat:@"%@%@", appname, NSLocalizedStringFromTable(@"SETTING_FEEDBACK_TITLE", @"RTService", @"")];
    return title;
}
...
-(void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    if (alertView.tag == REVIEW_REQUEST_ALERTVIEW_TAG) {
        switch (buttonIndex) {
            case 1:
#ifdef LOG_USER_ACTION
                [Flurry logEvent:@"RT Return" withParameters:@{@"Show":@(2)}];
#endif
                [self doRT];
                result = 1;
                break;
            case 2:
#ifdef LOG_USER_ACTION
                [Flurry logEvent:@"RT Return" withParameters:@{@"Show":@(-1)}];
#endif
