...
@interface CKAlertViewController ()
{
    ...
    UIEdgeInsets _rememberCheckMargin;
    CGFloat _rememberCheckWidth;
    CGFloat _rememberHeight;
    BOOL _withRemeberCheck;
}
...
- (void)viewDidLoad {
    ...
    if (_withRemeberCheck) {
        [self creatRememberView];
    }
...
- (void)defaultSetting {
    ...
    _rememberCheckMargin = UIEdgeInsetsMake(0, 0, 0, 4);
    _rememberCheckWidth = 30;
    _rememberHeight = 30;
    _withRemeberCheck = YES;
}
...
- (void)creatRememberView
{
    if (!_rememberLabel) {
        _rememberLabel = [self creatLabelWithFontSize:12];
        _rememberLabel.text = self.remember;
        _rememberLabel.textAlignment = NSTextAlignmentRight;
        [_contentView addSubview:_rememberLabel];
    }
    
    if (!_rememberBtn) {
        _rememberBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        _rememberBtn.showsTouchWhenHighlighted = NO;

        [_rememberBtn setImage:[UIImage imageNamed:@"矩形-33"] forState:UIControlStateNormal];
        [_contentView addSubview:_rememberBtn];
        [_rememberBtn addTarget:self action:@selector(checkBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    }
}
...
-(void)checkBtnClick:(UIButton *)btn
{
    ...
    [_rememberBtn setImage:[UIImage imageNamed:(btn.selected?@"矩形-34":@"矩形-33")] forState:UIControlStateNormal];
}
...
- (void)updateRememberFrame
{
    CGFloat labelWidth = _contentViewWidth - _contentMargin.left - _contentMargin.right - _rememberCheckMargin.left - _rememberCheckMargin.right - _rememberCheckWidth;

    CGFloat originY = [self getFirstButtonY] - 1 - _rememberHeight;

    if (self.rememberLabel.text.length) {
        CGSize size = [self.rememberLabel sizeThatFits:CGSizeMake(labelWidth, _rememberHeight)];
        CGFloat originX = _contentViewWidth - (_contentMargin.right + _rememberCheckMargin.left + size.width);
        self.rememberLabel.frame = CGRectMake(originX, originY, size.width, _rememberHeight);
        
        self.rememberBtn.frame = CGRectMake(originX-_rememberCheckMargin.right-_rememberCheckWidth, CGRectGetMinY(self.rememberLabel.frame), _rememberCheckWidth, _rememberHeight);
    }
}
...
- (CGFloat)getFirstButtonY {
    ...
    firstButtonY += _withRemeberCheck?_rememberCheckMargin.top+_rememberCheckMargin.bottom+_rememberHeight:0;
    return firstButtonY;
}
...
- (void)didClickButton:(UIButton *)sender {
    ...
    [self showDisappearAnimationWithHandler:^{
...
{
    return self.rememberBtn.selected;
}
...
- (void)showDisappearAnimationWithHandler:(void (^)(void))handler; {
...
-(void)setRemember:(NSString *)remember
{
    ...
    _withRemeberCheck = remember.length>0;
}
