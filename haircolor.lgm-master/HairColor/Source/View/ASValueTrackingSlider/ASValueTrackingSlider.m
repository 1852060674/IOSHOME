...
-(ASValueTrackingSlider *)initWithFrame:(CGRect)frame andVertical:(BOOL)isVertical
{
...
-(void)setPopupContainerView:(UIView *)view
{
...
- (void)setup
{
    ...
    self.popUpView.isVertical = _isVertical;
...
//    [self.popUpView setTextLayerTransform:CGAffineTransformIdentity];
    
    popUpRect = [self.popUpView.superview convertRect:popUpRect fromView:self];
    [self.popUpView setFrame:popUpRect arrowOffset:offset text:valueString];
    [self.popUpView.superview bringSubviewToFront:self.popUpView];
//    [self.popUpView setTextLayerTransform:CGAffineTransformMakeRotation(M_PI_2)];
}
