...
- (void)setTextLayerTransform:(CGAffineTransform)transform
{
//    _textLayer.anchorPoint = CGPointMake(CGRectGetWidth(_textLayer.bounds)*0.5, CGRectGetHeight(_textLayer.bounds)*0.5);
...
- (CGSize)popUpSizeForString:(NSString *)string
{
    ...
    if (_isVertical) {
        w = ceilf([_attributedString size].height * _heightPaddingFactor);
        h = ceilf(([_attributedString size].width * _widthPaddingFactor) + _arrowLength);
    }
    else
    {
...
- (UIBezierPath *)pathForRect:(CGRect)rect withArrowOffset:(CGFloat)arrowOffset;
{
    ...
    if (_isVertical) {
        //     Create rounded rect
        roundedRect.size.width -= _arrowLength;
        
        // Create arrow path
        CGFloat maxY = CGRectGetMaxY(roundedRect); // prevent arrow from extending beyond this point
        CGFloat arrowTipY = CGRectGetMidY(rect) - arrowOffset;
        CGPoint tip = CGPointMake(CGRectGetMaxX(rect), arrowTipY);
        
        CGFloat arrowLength = CGRectGetWidth(roundedRect)/2.0;
        CGFloat y = arrowLength * tan(45.0 * M_PI/180); // x = half the length of the base of the arrow
        
        [arrowPath moveToPoint:tip];
        [arrowPath addLineToPoint:CGPointMake(CGRectGetMaxX(roundedRect) - arrowLength, MIN(arrowTipY + y, maxY))];
        [arrowPath addLineToPoint:CGPointMake(CGRectGetMaxX(roundedRect) - arrowLength, MAX(arrowTipY - y, 0))];
        [arrowPath closePath];
...
- (void)layoutSubviews
{
    ...
    if (_isVertical) {
        CGRect textRect = CGRectMake(0,
                                     (self.bounds.size.height-textHeight)/2,
                                     self.bounds.size.width-_arrowLength, textHeight);
