...
#include "InternalSharedMatting.hpp"
InternalSharedMatting::InternalSharedMatting()
{
    kI = 10;
    kC = 5.0;
    kG = 4;
...
//void InternalSharedMatting::loadImage(const char * filename)
void InternalSharedMatting::loadImage(cv::Mat image)
{
...
//void InternalSharedMatting::loadTrimap(const char * filename)
void InternalSharedMatting::loadTrimap(cv::Mat tripMat)
{
    tripMat.copyTo(trimap);
}

void InternalSharedMatting::expandKnown()
{
    vector<struct labelPoint> vp;
    int kc2 = kC * kC;
    vp.clear();
    int s       = (int)trimap.step[0];
    ...
    for (int i = 0; i < height; ++i)
    {
        for (int j = 0; j < width; ++j)
        {
            ...
            if (tri[i][j] != 0 && tri[i][j] != 255)
            {
                ...
                {
                    /// note: uT - ALL unknow pixels after expansion of know regions
                    CvPoint lp;
                    lp.x = i;
                    lp.y = j;
                    uT.push_back(lp);
...
double InternalSharedMatting::comalpha(CvScalar c, CvScalar f, CvScalar b)
{
...
double InternalSharedMatting::mP(int i, int j, CvScalar f, CvScalar b)
{
...
double InternalSharedMatting::nP(int i, int j, CvScalar f, CvScalar b)
{
    int i1 = max(0, i - 1);
...
double InternalSharedMatting::eP(int i1, int j1, int i2, int j2)
{
...
double InternalSharedMatting::pfP(CvPoint p, vector<CvPoint>& f, vector<CvPoint>& b)
{
    double fmin = 1e10;
...
double InternalSharedMatting::aP(int i, int j, double pf, CvScalar f, CvScalar b)
{
    int bc = data[i * step + j * channels];
    int gc = data[i * step + j * channels + 1];
    int rc = data[i * step + j * channels + 2];
...
double InternalSharedMatting::dP(CvPoint s, CvPoint d)
{
    return sqrt(double((s.x - d.x) * (s.x - d.x) + (s.y - d.y) * (s.y - d.y)));
}

double InternalSharedMatting::gP(CvPoint p, CvPoint fp, CvPoint bp, double pf)
{
    /// note: compute cost function
    int bc, gc, rc;
...
double InternalSharedMatting::gP(CvPoint p, CvPoint fp, CvPoint bp, double dpf, double pf)
{
    int bc, gc, rc;
    bc = data[fp.x * step + fp.y * channels];
    gc = data[fp.x * step + fp.y * channels + 1];
...
double InternalSharedMatting::sigma2(CvPoint p)
{
    int xi = p.x;
    int yj = p.y;
    int bc, gc, rc;
...
double InternalSharedMatting::distanceColor2(CvScalar cs1, CvScalar cs2)
{
...
void InternalSharedMatting::sample(CvPoint p, std::vector<CvPoint> &f, std::vector<CvPoint> &b)
{
    int i = p.x;
    int j = p.y;
...
void InternalSharedMatting::Sample(std::vector<vector<CvPoint> > &F, std::vector<vector<CvPoint> > &B)
{
    int   a,b,i;
    int   x,y,p,q;
    int   w,h,gray;
    ...
    {
        /// note: for each unknown pixel, search for bg & fg pixels. search starts from unknown pixel along some segment and picks the first fg/bg pixels encounted as samples.
...
void InternalSharedMatting::gathering()
{
    vector<CvPoint> f;
    vector<CvPoint> b;
    vector<CvPoint>::iterator it;
    ...
    int index = 0;
    double a;
    int size = (int)uT.size();
    
    for (int m = 0; m < size; ++m)
    {
        /// note: for each unknown pixel
        int i = uT[m].x;
        int j = uT[m].y;
        ...
        /// note: find the pair which minimize gp(the cost function defined by Equation 7 in the paper)
...
void InternalSharedMatting::refineSample()
{
    ...
    for (it = uT.begin(); it != uT.end(); ++it)
    {
        ...
        /// note: get the three tuples among neighbours with the lowest values of Mp and average them
...
void InternalSharedMatting::localSmooth()
{
    vector<CvPoint>::iterator it;
...
void InternalSharedMatting::save(const char * filename)
{
    
//    cvSaveImage(filename, matte);
}

void InternalSharedMatting::getMat(cv::Mat &alphaMat)
{
    int h     = height;
    int w     = width;
    
    alphaMat.create(h, w, CV_8UC1);

    uchar *d;
    for (int i = 0; i < h; ++i)
    {
        d = alphaMat.ptr<uchar>(i);
        for (int j = 0; j < w; ++j)
        {
...
void InternalSharedMatting::solveAlpha()
{
