...
- (NSArray *)builtInCardBackImageNames {
  return [NSArray arrayWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"cardback" ofType:@"plist"]];
}

- (NSArray *)builtInDeskBackImageNames {
  return [NSArray arrayWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"background" ofType:@"plist"]];
}
...
- (void)removeCustomBg:(NSString *)name {
  ...
  NSString * defaultName = isCardBg?@"cardback1.png":@"bg0";
  if (bgsM.count == 0) {
  ...
  if (_leftB.selected) {
    ...
    if (indexPath.item < bgs.count) {
      ...
      if (self.isRemovingCustom) {
        ...
        NSString * imgname = bgs[indexPath.item];
        [self setPrefObj:@(NO) forKey:@"userdefined-background"];
