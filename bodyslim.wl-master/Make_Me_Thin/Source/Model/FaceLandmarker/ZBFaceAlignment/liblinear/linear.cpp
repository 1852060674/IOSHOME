	...
	float Cp, float Cn, int solver_type)
{
	...
	float diag[3] = {(float)0.5/Cn, 0, (float)0.5/Cp};
	...
	while(newton_iter < max_newton_iter)
	{
		...
		while(iter < max_iter)
		{
			...
			for(s=0; s<QP_active_size; s++)
			{
				...
				if(fabs(z) < 1.0e-12)
					continue;
				z = min(max(z,(float)-10.0),(float)10.0);
...
int save_model_binary(FILE *fp, const struct model *model_)
{
    if(fp==NULL) return -1;
    const parameter& param = model_->param;
    ...
    fwrite(&param.solver_type, sizeof(int), 1, fp);
    fwrite(&model_->nr_class, sizeof(int), 1, fp);
    
    bool has_label = (model_->label!=NULL);
    fwrite(&has_label, sizeof(bool), 1, fp);
    if(has_label)
    {
        fwrite(model_->label, sizeof(int), model_->nr_class, fp);
    }
    
    fwrite(&model_->nr_feature, sizeof(int), 1, fp);
    fwrite(&model_->bias, sizeof(float), 1, fp);

    fwrite(model_->w, sizeof(float), w_size*nr_w, fp);

    return 0;
}

struct model *load_model_binary(FILE *fp)
{
    model *model_ = Malloc(model,1);
    parameter& param = model_->param;
    ...
    fread(&param.solver_type, sizeof(int), 1, fp);
    fread(&model_->nr_class, sizeof(int), 1, fp);
    
    bool has_label;
    fread(&has_label, sizeof(bool), 1, fp);
    if (has_label) {
        model_->label = Malloc(int,model_->nr_class);
        fread(model_->label, sizeof(int), model_->nr_class, fp);
    }
    
    fread(&model_->nr_feature, sizeof(int), 1, fp);
    fread(&model_->bias, sizeof(float), 1, fp);
    ...
    fread(model_->w, sizeof(float), w_size*nr_w, fp);

    return model_;
}

struct model *load_model_binary_buffer(unsigned char **bufferPtr)
{
    ...
    memcpy(&param.solver_type, buffer, amount);
    buffer += amount;
    
    amount = sizeof(int);
    memcpy(&model_->nr_class, buffer, amount);
    buffer += amount;
    
    bool has_label;
    amount = sizeof(bool);
    memcpy(&has_label, buffer, amount);
    buffer += amount;
    
    if (has_label) {
        model_->label = Malloc(int,model_->nr_class);
        amount = sizeof(int)*model_->nr_class;
        memcpy(&model_->label, buffer, amount);
        buffer += amount;
    }
    
    amount = sizeof(int);
    memcpy(&model_->nr_feature, buffer, amount);
    buffer += amount;
    
    amount = sizeof(float);
    memcpy(&model_->bias, buffer, amount);
    buffer += amount;
    ...
    amount = sizeof(float)*w_size*nr_w;
    memcpy(model_->w, buffer, amount);
    buffer += amount;
