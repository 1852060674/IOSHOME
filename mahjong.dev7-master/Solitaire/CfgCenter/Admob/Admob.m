...
- (id) init
{
    ...
    //11.16 move  [self init_reward_ad];
    return self;
}
...
-(void) doUpgradeInApp:(UIViewController*)viewCtrl product:(NSString*)productID
{
    ...
    iapv.view.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.1];
    [viewCtrl presentViewController:iapv animated:YES completion:^{
        ;
    }];
#endif
...
-(void) init_reward_ad {
    ...
    // 11.16 move to self.load_reward_ad
...
//
- (void)setRewardAdClient:(RewardAdWrapperDelegate *)AdClient {
    ...
    @try {
        ...
        if([appid length] != 0) {
            ...
            UIAlertView *rtDlg = [[UIAlertView alloc]
                                  initWithTitle:@""
                                  message:message
                                  delegate:self cancelButtonTitle:d
