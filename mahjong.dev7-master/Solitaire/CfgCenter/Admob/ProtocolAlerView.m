...
//    self.textView.frame = CGRectMake(margin, CGRectGetMaxY(self.titleL.frame) +margin *0.7, width -margin *2, height - margin *5);
    ...
    self.textView.frame = CGRectMake(margin, CGRectGetMaxY(self.titleL.frame) +margin *0.7- margin, width -margin *2, height - margin *3);
    
//    self.agreeBtn.frame =
//    CGRectMake(margin, height - margin *5, width - margin *2, margin *2);
    /// update zzx 11.21  按钮大小变小
    self.agreeBtn.frame =
    CGRectMake(width/2-3*margin, height - margin * 1.5, (width - margin *2)/2, (margin *2)/2);
...
//        _contentView.frame = CGRectMake(leftMargin, topMargin*1.5, kScreenWidth - leftMargin *2, kScreenHeight - topMargin *4.6);
        /// updte zzx 11.21 update 横屏下弹窗大小调整
        CGFloat x=4.5;
        if ([self isNotchScreen]) {
            x=3.8;
        }
        _contentView.frame = CGRectMake(leftMargin, topMargin*2, kScreenWidth - leftMargin *2, kScreenHeight - topMargin *x);
