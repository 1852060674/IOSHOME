...
-(NSArray*) getAdConfigList:(BOOL)usdefault {
    if(usdefault)
        return defaultadconfig_list;
    return adconfig_list;
}
...
-(void)udconfig: (char*) data length: (long)length {
    ...
    @try {
        ...
        adconfig_list = [jsonDict objectForKey:@"_config"];
...
-(void)parseDefaultConfig: (NSString*) data {
    ...
    defaultadconfig_list = [jsonDict objectForKey:@"_config"];
