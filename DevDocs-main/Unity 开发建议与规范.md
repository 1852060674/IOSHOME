...
public void SetCurrentHealth(float currentHealth) => _currentHealth = Mathf.Clamp01(currentHealth);
```
...
```
每多一层嵌套，你的大脑就需要多记住一个上下文条件。读到最内层时，你必须同时记住 `i` 的值以及 `conditionA`、`conditionB` 和 `conditionC` 都为 `true`。这极大地增加了理解代码的 “认知负担” 。
...
“魔法值” 是指在代码中直接使用的、没有经过任何解释或命名的硬编码数值。它就像一张来历不明的纸条，上面只写着一个数字 “7” 。这个 “7” 是什么意思？是代表一周有 7 天？玩家最多能带 7 个道具？还是角色的攻击力是 7 ？没人知道。它就像魔法一样凭空出现，充满了神秘感，所以我们叫它 “魔法值” 。

虽然它们在短期内可能看起来方便，但在长期维护中往往会带来问题，因为数字的意义可能对阅读代码的人来说不清晰。例如：
```csharp
// 充满魔法值的代码
...
![图片1](Unity%20开发建议与规范/Image_1.png)
## 空引用检查
...
![图片5](Unity%20开发建议与规范/Image_5.png)

尤其在处理泛型或者 `System.Object` 的空引用检查时容易被忽略，此时并不会使用重载后的操作符。可以使用扩展方法自定义空引用检查而不使用 `==` 和 `!=` 运算符。
```csharp
public static bool IsNull([NotNullWhen(false)] this object? obj) => obj == null || (obj is UnityObject unityObject && unityObject == null);
```
## 格式化字符串
...
```
从输出可以清晰地看到在模拟的德国环境下，98.6 变成了 98,6 。如果这个字符串被保存，其他地区的玩家加载时，`float.Parse("98,6")` 如果不指定 `CultureInfo` ，很可能会失败或解析成 986。使用 `CultureInfo.InvariantCulture` 生成的字符串 98.6 始终保持一致，可以被任何系统安全地解析。
...
```
![图片4](Unity%20开发建议与规范/Image_4.png)
...
```
![图片6](Unity%20开发建议与规范/Image_6.png)
...
```
![图片7](Unity%20开发建议与规范/Image_7.png)
## 资源管理
...
使用 Addressables，你只需要把 Prefab 标记为 "Addressable"，系统会自动分析它的整个依赖链（材质、贴图、Shader、动画...），然后智能地将依赖项打包并且检测是否存在重复的资源。
...
```csharp
// 加载逻辑
private void LoadSprite(string address)
{
	// 使用 Addressables.LoadAssetAsync 来加载资源数据
	spriteHandle = Addressables.LoadAssetAsync<Sprite>(address);
	spriteHandle.Completed += handle =>
	{
		if (handle.Status == AsyncOperationStatus.Succeeded)
		{
			Debug.Log($"Sprite '{address}' 加载成功!");
			// 将加载的 Sprite 赋值给 Image
			targetImage.sprite = handle.Result;
		}
		else
		{
			Debug.LogError($"Sprite '{address}' 加载失败: {handle.OperationException}");
		}
	};
}
...
void OnDestroy()
{
	Debug.Log("开始释放所有已加载的 Addressable 资源...");

	// 使用 Addressables.Release 并传入保存的句柄来减少资源的引用计数。
	// 当一个资源的引用计数降为 0 时，它占用的内存才会被真正释放。
	if (spriteHandle.IsValid())
	{
		Addressables.Release(spriteHandle);
		Debug.Log("Sprite handle released.");
	}
}
```

Addressables 在不同的机器环境中打包可能计算出不同的 Hash 值，尽量在同一台机器上打包。
## 动画
...
entrySequence
    ...
    // 同时，从 0.8 倍大小放大到 1 倍，使用 Ease.OutBack 产生回弹效果
...
```
实现相对复杂的动画需要使用到 Unity 的 `AnimationClip`，但是 Mecanim 动画系统使用起来相当繁琐。任何一个新的动画或行为，通常都需要打开 Animator 窗口，创建新的 State，设置新的 Parameter，再拉好所有的 Transition 连线。当角色技能繁多（例如有 50 个技能的 RPG 角色）时，Animator Controller 会变成一张巨大且难以维护的 “意大利面条式” 的灾难图。
...
```
尽量避免将重要的、核心的游戏逻辑放在动画事件的回调中。

把你的游戏逻辑（比如扣血、加分、改变游戏状态）和动画表现（一个 .anim 文件）紧紧地绑在一起。如果有一天你想在没有这个动画的情况下触发逻辑，或者复用这个动画但执行不同逻辑，都会变得非常困难。

动画不应该驱动逻辑本身，例如点击按钮后播放金币飞向收集点的动画，到达收集点后触发事件增加金币的数值，将结算逻辑放在动画事件中，会引入大量潜在的 Bug 和漏洞。如果玩家在金币飞行的过程中，游戏卡顿、切换场景、弹出广告、或者关闭游戏，这个动画就无法正常播放到结尾。结果就是：**玩家的金币永远不会到账**。这是一个非常严重的 Bug 。

应该先处理逻辑，再播放动画。动画系统的核心职责是 “表现” ，而游戏逻辑的职责是 “控制” 。

动画事件并非一无是处，它非常适合处理那些与动画表现本身紧密相关、非核心的 “效果” 。比如播放音效、触发特效等，即使它们没有被成功触发，核心的游戏逻辑也不会受到影响。
## 异步任务
使用 UniTask 执行异步任务，UniTask 是一个为 Unity 专门设计和优化的、高性能的开源异步解决方案。它旨在完全取代协程和在 Unity 中直接使用 `Task`。它基于 `struct` 设计，能极大减少 GC（垃圾回收）产生。

协程无法直接返回一个值 ( `IEnumerator` 的返回类型是固定的)。你需要通过回调函数或引用传参等笨拙的方式来获取结果。UniTask 使用 `UniTask<T>`，可以像普通函数一样自然地 `return` 结果。

你无法在 `try-catch` 块中包裹一个 `yield return` 来捕获异步操作中发生的异常。而 UniTask 基于 `async/await`，可以完全使用标准的 `try-catch` 语法，代码健壮性天差地别。

协程无法直接获取结果，也无法 `try-catch` ：
```csharp
void Start()
{
...
`Task` 的 `await` 之后，代码可能会在任意一个后台线程上恢复执行。如果你此时无法访问 Unity API（如 `transform.position` ），你需要手动处理线程同步。而 UniTask 默认总是在 Unity 主线程上恢复执行，这正是 Unity 开发所需要的行为，安全且直观。
...
```
## 避免过度设计
过度设计是 “未来的敌人” ，它试图用今天复杂的代码去解决明天不一定存在的问题，最终往往导致当前项目举步维艰。

你花了几周时间去构建一个 “完美” 的、可扩展的背包系统，它支持分页、排序、过滤、皮肤更换…… 但实际上，当前的游戏原型只需要能捡起一把钥匙并用它开门。这几周的时间本可以用来开发更多核心玩法，更快地验证游戏是否好玩。

例如当前只需要一个 “火球术” ，那就先实现一个 `Fireball` 类。当未来需要 “冰箭术” 时，再回头看这两个技能的共同点，提炼出一个 `Projectile` 基类。这比一开始就设计一个庞大复杂的 `SkillSystem` 要高效得多。

代码不是一成不变的，随着新需求的增加，代码变 “坏” 是正常的。我们应该定期安排时间进行重构。当简单的系统无法满足新需求时，就是重构和提升设计的好时机。

凭借经验判断哪些地方是未来会扩展的 “业务核心” ，值得稍微多花一点心思，而哪些地方是 “边缘功能” ，用最直接的方式实现即可。