...
//    vc = (ViewController*)[(UINavigationController*)[[[UIApplication sharedApplication] keyWindow] rootViewController] topViewController];
    ...
    for (int i = 0; i < [backNameByIndex count]; i++) {
        ...
        if (userdefined && i + 1 == [backNames count]) {
            ...
            if (![vc OldMan]) {
                 imgName = [NSString stringWithFormat:@"%@/Documents/%@.jpg",NSHomeDirectory(), [backNames lastObject]];
            }
...
//    self.scrollView.frame=frame;
    if ([selectedBackName hasPrefix:@"cardback"]) {//zzx 0328
        selectedBackName = @"CardBack-BlueGrid";
    }
...

{
    ...
    [image drawInRect:CGRectMake(0, 0, reSize.width-200, reSize.height)];
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary*)info
{
    ...
    NSLog(@"selectedBackName imgName 12 =%@",imgName);
