...
@interface ChangeBackgroundViewController ()
{
    ...
    __weak IBOutlet NSLayoutConstraint *bottomViewTop;
...
- (void)viewDidLoad
{
    if (kScreenHeight > 811 || kScreenWidth >811) {
        bottomViewTop.constant=55;
        if (kScreenHeight >851) {
            bottomViewTop.constant=65;
        }
    }
    [super viewDidLoad];
...
- (void)setLayoutImags
{
    for (int i = 0; i < [backNameByIndex count]; i++) {
        ...
        if (userdefined && i + 1 == [backNames count]) {
            ...
            NSString *imgName = [NSString stringWithFormat:@"%@/Documents/%@%@.png",NSHomeDirectory(), [backNames lastObject],retinaStr];
            if (![vc OldMan]) {
                imgName = [NSString stringWithFormat:@"%@/Documents/%@%@.jpg",NSHomeDirectory(), [backNames lastObject],retinaStr];
            }
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary*)info
{
    ...
    UIImage* scaleImage = [self reSizeImage:image toSize:CGSizeMake([UIApplication currentSize].width*scale, [UIApplication currentSize].height*scale)];
    NSString *retinaStr = @"";
    if (scale == 2.0)
        retinaStr = @"@2x";
    ...
    // last
    NSLog(@"selectedBackName imgName 1 =%@",selectedBackName);
    NSInteger index = [backNames indexOfObject:selectedBackName];
    ...
    NSLog(@"selectedBackName imgName 2 =%@",imgName);
    self.backImg.image = [UIImage imageWithContentsOfFile:imgName];
...
{
    BOOL rflag = [[NSUserDefaults standardUserDefaults] boolForKey:@"orientation"];
    if (rflag && [[[UIDevice currentDevice] systemVersion] floatValue] < 8.0)
    {
        [self layoutPics];
    }
    return [[NSUserDefaults standardUserDefaults] boolForKey:@"orientation"];
...
- (void)picSelected:(NSNotification*)notifacation
{
    ...
    NSLog(@"test zzx 0 setObject:selectedBackName forKey:background %@",OldBack);
