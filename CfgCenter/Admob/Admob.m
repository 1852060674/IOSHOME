...
#ifdef ENABLE_AD
        //init ad network
        [ADWrapper initAdNetwork:^(){
            if(self->adcenter != nil) {
                [self->adcenter delayInitAfterNetworkFinish];
            }
            if(self->adcenter_land != nil) {
                [self->adcenter_land delayInitAfterNetworkFinish];
            }
            if(self->rewardad != nil) {
                [self->rewardad delayInitAfterNetworkFinish];
            }
            if(self->openad != nil) {
                [self->openad delayInitAfterNetworkFinish];
            }
        }];
...
- (void)interstitialDidReceiveAd
{
    ...
    if([self.interadClient respondsToSelector:@selector(adMobVCDidReceiveInterstitialAd:)]) {
        [self.interadClient adMobVCDidReceiveInterstitialAd:self];
    }
}
...
-(BOOL)interstitialDidDismissScreen
{
    ...
    if([self.interadClient respondsToSelector:@selector(adMobVCDidCloseInterstitialAd:)]) {
        [self.interadClient adMobVCDidCloseInterstitialAd:self];
    }
...
-(void)interstitialWillDismissScreen
{
    ...
    if ([self.interadClient respondsToSelector:@selector(adMobVCWillCloseInterstitialAd:)]) {
        [self.interadClient adMobVCWillCloseInterstitialAd:self];
    }
}
...
-(void)interstitialDidFailed
{
    ...
    if ([self.interadClient respondsToSelector:@selector(adMobVCDidFailedInterstitialAd:)]) {
        [self.interadClient adMobVCDidFailedInterstitialAd:self];
    }
}
...
-(void)interstitialDidShow
{
    ...
    if ([self.interadClient respondsToSelector:@selector(adMobVCDidShowInterstitialAd:)]) {
        [self.interadClient adMobVCDidShowInterstitialAd:self];
    }
}
...
-(void)interstitialDidClick
{
    ...
    if ([self.interadClient respondsToSelector:@selector(adMobVCDidClickInterstitialAd:)]) {
        [self.interadClient adMobVCDidClickInterstitialAd:self];
    }
}
...
- (void)init_open_ad {
    @try {
        openad = [OpenAdWrapper createAD:[configCenter getOpenAdConfig:false] vc:self];
    } @catch (NSException *exception) {
    } @finally {
    }
    ...
    //consider the build in value is alway valid
    if(openad == nil)
        openad = [OpenAdWrapper createAD:[configCenter getOpenAdConfig:true] vc:self];
    
    openad.delegate = _openAdClient;
    
    [openad init_ad];
}

- (BOOL)isOpenAdLoaded:(int)place {
    if(openad != nil) {
        return [openad isAdReady:place];
    }
    return FALSE;
}

- (BOOL)showOpenAd:(UIViewController *)rootview placeid:(int)place {
    if(openad != nil) {
        return [openad showAd:rootview placeid:place];
    }
    return FALSE;
}
