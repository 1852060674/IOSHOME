...
-(NSDictionary*) getOpenAdConfig:(BOOL)usdefault {
    if(usdefault)
        return defaultOpenAd;
    return openAdConfig;
}
...
-(void)udconfig: (char*) data length: (long)length {
    ...
    @try {
        ...
        openAdConfig = [jsonDict objectForKey:@"openad"];
...
-(void)parseDefaultConfig: (NSString*) data {
    ...
    defaultOpenAd = [jsonDict objectForKey:@"open"];
