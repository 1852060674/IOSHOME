...
{    
    ...
    //UIImageWriteToSavedPhotosAlbum(realImage, self, nil, nil);    
    */
    return tvImgWithBG;
}
...
{
    return [self mergeImage:bg front:front size:size alpha:0.3];
}
...
+ (UIImage*)draw3_2_Image:(UIImage*)image withPoints:(NSArray*)pointArray withCenter:(CGPoint)centerPoint radius:(CGFloat)radius startAngle:(CGFloat)startAngle endAngle:(CGFloat)endAngle clockwise:(BOOL)clock
{
    ...
    
    {
        ...
        [aPath addArcWithCenter:centerPoint radius:radius startAngle:startAngle endAngle:endAngle clockwise:clock];
        
        _value = [pointArray objectAtIndex:4];
        [aPath addLineToPoint:[_value CGPointValue]];
        
        _value = [pointArray objectAtIndex:5];
        [aPath addLineToPoint:[_value CGPointValue]];
