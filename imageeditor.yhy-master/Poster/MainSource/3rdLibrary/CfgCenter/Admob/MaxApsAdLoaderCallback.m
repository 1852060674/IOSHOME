...
- (void)addMaxaAdwardAdObj:(ApplovinMaxRewardWrapper*) applovin_ad_obj {
    [ad_list addObject:applovin_ad_obj];
}

- (void)onSuccess:(DTBAdResponse *)adResponse {
    NSLog(@"Are zzx onSuccess");
    if(ad_type == 1) {
...
- (void)onFailure:(DTBAdError)error dtbAdErrorInfo:(DTBAdErrorInfo *)dtbAdErrorInfo {
    ...
    if(ad_type == 1) {
        ...
        for(int i=0; i<ad_list.count; i++) {
            NSLog(@"zzx onrewardAPSFailure execute");
            [ad_list[i] onrewardAPSFailure:error dtbAdErrorInfo:dtbAdErrorInfo];
        }
    }
}
