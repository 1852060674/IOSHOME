...
-(BOOL) showRewardAd:(UIViewController*)viewController placeid:(int)place
{
    ...
    if (rewardedAd != NULL) {
        ...
        [rewardedAd presentFromRootViewController:viewController userDidEarnRewardHandler:^{
            ...
            [FIRAnalytics logEventWithName:[self adPosition] parameters:@{@"id": reward_id, @"status": @"reward"}];
#endif
...
#ifdef LOG_USER_ACTION
        [FIRAnalytics logEventWithName:[self adPosition] parameters:@{@"id": reward_id, @"status": @"show"}];
#endif
        isshowing = true;
        return YES;
    }
...
-(void) _rewardAdReload {
    ...
    [GADRewardedAd loadWithAdUnitID:reward_id request:[self getADRequest] completionHandler:^(GADRewardedAd *ad, NSError *error) {
            if (error) {
            ...
            #ifdef LOG_USER_ACTION
                [FIRAnalytics logEventWithName:[self adPosition] parameters:@{@"id": reward_id, @"status": @"failed"}];
            #endif
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(30.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    if(rewardAdRetryCount < 3) {
                        ...
                        NSLog(@"[ADUNION] retry admob reward ad1 %d!", rewardAdRetryCount);
                    }
                    else {
                        rewardAdRetryCount = 0;
                        isloading = false;
            ...
            #ifdef LOG_USER_ACTION
                [FIRAnalytics logEventWithName:[self adPosition] parameters:@{@"id": reward_id, @"status": @"loaded"}];
            #endif
            }
        }];
#ifdef LOG_USER_ACTION
    [FIRAnalytics logEventWithName:[self adPosition] parameters:@{@"id": reward_id, @"status": @"load"}];
#endif
}
...
#ifdef LOG_USER_ACTION
    [FIRAnalytics logEventWithName:[self adPosition] parameters:@{@"id": reward_id, @"status": @"click"}];
#endif
...
#ifdef LOG_USER_ACTION
    [FIRAnalytics logEventWithName:[self adPosition] parameters:@{@"id": reward_id, @"status": @"closed"}];
#endif
