...
@implementation ApplovinMaxRewardWrapper
{
    ...
    DTBAdResponse* rewardResponse;
    DTBAdErrorInfo* rewardErrorInfo;
}
...
- (void) createRewardAd {
    ...
    // Load the first ad
    if(rewardResponse != nil) {
        NSLog(@"createInterstitialAd execute");
        [rewardedAd setLocalExtraParameterForKey:@"amazon_ad_response" value:rewardResponse];
    } else if(rewardErrorInfo != nil) {
        [rewardedAd setLocalExtraParameterForKey:@"amazon_ad_error" value:rewardErrorInfo];
    }
...
- (void)didLoadAd:(MAAd *)ad
{
    if([ad.adUnitIdentifier isEqualToString: reward_id]) {
        ...
        [self send_flurry_report:reward_id status:@"loaded"];
...
- (void)onrewardAPSSuccess:(DTBAdResponse *)adResponse {
    rewardResponse = adResponse;
    if(reward_delay_init && applovin_initialized) {
        reward_delay_init = false;
        if(reward_id && ![reward_id isEqualToString:@""]) {
            [self createrewardAdinfo];
        }
    }
    rewardCallback.isInUse = NO;
}
- (void)onrewardAPSFailure:(DTBAdError)error dtbAdErrorInfo:(DTBAdErrorInfo *)dtbAdErrorInfo {
    rewardErrorInfo = dtbAdErrorInfo;
    if(reward_delay_init && applovin_initialized) {
        reward_delay_init = false;
        if(reward_id && ![reward_id isEqualToString:@""]) {
//            [self createRewardAd];
...
- (void) createrewardAdinfo {
    rewardedAd = [[MAInterstitialAd alloc] initWithAdUnitIdentifier: reward_id];
    rewardedAd.delegate = self;
    rewardedAd.revenueDelegate = _revenueDelegate;
    // aps 没有返回信息
    if(rewardResponse != nil) {
